...
public class JetStreamAsyncNoAckProducer {
    private static final Logger log = LoggerFactory.getLogger(JetStreamAsyncNoAckProducer.class);
    ...
    public static void main(String[] asArgs) throws Exception {
        ...
        JetStreamOptions jsOpt = JetStreamOptions.builder().publishNoAck(false).build();
        JetStream jetStream = nc.jetStream(jsOpt);
        ...
        try {
            ...
            for( int i=0 ; i != iter ; i++ ) {
                ...
                while( (et = System.nanoTime()) - st < loop_ns) {
                    try {
                        // asyncでFutureがたまりすぎるので制限
                        if( latch.get() < 100000) {
                            jetStream.publishAsync(topic, serializer.serialize(null, new LatencyMeasurePing(seq)));
                            seq++;
                            cnt++;
                        }else {
                            Thread.yield();
